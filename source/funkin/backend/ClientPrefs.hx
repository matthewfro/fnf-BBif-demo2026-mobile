...
class ClientPrefs
{
	...
	public static function loadPrefs()
	{
		...
		if (FPSCounter.instance != null) FPSCounter.instance.visible = data.showFPS;
		...
		if (FlxG.save.bind(name, CoolUtil.getSavePath()) == false) // coudlnt bind the save so just fallback
		{
			@:privateAccess
			{
				...
				if (FileSystem.exists(path))
				{
					...
					FlxG.stage.window.alert('Save was corrupted. corrupted save was placed at $corruptedPath', 'Corrupt Save File');
